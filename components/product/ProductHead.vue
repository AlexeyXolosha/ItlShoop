        ...
        <div class="section__body">
            <div class="product__head">
                <div class="product__wrapper">
                        ...
                        <div class="product__title">
                            <ul class="product-title__list">
                                <li class="product-title__item" v-for="(item, index) in props.props?.meta?.breadcrumb"
                                    :key="index">
                                    <a :href="item.links?.self" class="product-title__link">
                                        <span>{{ item.attributes.name }}</span>
                                    </a>
                                    <span v-if="index !== props.props.meta.breadcrumb.length - 1">/</span>
                                </li>
                            </ul>

                            <div class="product__head-middle">
                                <h1 class="product__head-title">
                                    {{ props.props.data.attributes.name }}
                                </h1>
                                <div class="product__stickers">
                                    <span class="product__stiker"
                                        v-for="(stikers, index) in props.props.data.attributes?.stikers?.value"
                                        :key="index" :class="[stickerColors[stikers]]">{{ stikers }}</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <a :href="props.props.data.attributes.brand[0].links.self">
                        <img class="product__image" :src="imageUpload(props.props.data.attributes.brand[0].attributes.images.preview)" width="81" height="36"  alt="">
                    </a>
                </div>
            </div>
        </div>
